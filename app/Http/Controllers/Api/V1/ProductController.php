...
class ProductController extends Controller
{
    ...
    public function getTiketProduct(Request $request)
    {
        try {
            $limit = $request->limit ?? 10;
            $filter = $request->filter ?? [];
            $sorting = $request->sortby ?? null;
            return $this->productQueries->getTiketProduct($limit, $filter, $sorting, request()->input('page') ?? 1);
        } catch (Exception $e) {
            return $this->respondErrorException($e, request());
        }
    }

    public function getSubsidyProduct(Request $request)
    {
        try {
            $limit = $request->limit ?? 10;
            $filter = $request->filter ?? [];
